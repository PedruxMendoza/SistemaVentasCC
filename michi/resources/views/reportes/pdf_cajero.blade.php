...
<body>
    <header>
        <h1>Reporte de Ventas efectuadas</h1>
    </header>

    <main>
        <h1>Ventas efectuadas</h1>       
        @if (is_null($cashier) && is_null($inicio) && is_null($final))
        ...
        ?>
        @foreach($cajero as $caj) 
        <h2>por {{$caj->Nombre_Completo}} del {{$fechai->format('d M Y')}} al {{$fechaf->format('d M Y')}}</h2>
        @endforeach
        @endif 
        @endif  
        @endif         
        ...
        <table class="table-style-one">
            ...
            <tbody>
                ...
                @empty
                <tr>                    
                    <td colspan="4" align="center"><p>No existen Facturas, Por favor intente de nuevo!</p></td>
                </tr>
                @endforelse
            </tbody>
        </table>
